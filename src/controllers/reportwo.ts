...
export async function generateWorkOrderReport(c: Context) {
    try {
        ...
        processWorkOrderReport(csvJob.id, params, userId);
...
async function processWorkOrderReport(
    ...
    try {
     ...
     `;
        ...
        try {
            const partialFilePath = getReportPath(`workorders_${params.startDate}_${params.endDate}_${Date.now()}.csv`);
    ...
    try {
        ...
        return c.json({
            ...
            data: {
                ...
                fileSize: job.fileSize ? `${(Number(job.fileSize) / 1024 / 1024).toFixed(2)} MB` : null,
